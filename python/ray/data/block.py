...
class BlockAccessor:
    ...
    def random_shuffle(self, random_seed: int | tuple[int, ...] | None) -> Block:
        """Randomly shuffle this block.

        Args:
            random_seed: An integer or tuple of integers to seed
                ``np.random.default_rng()``, or None for non-deterministic
                shuffling. A tuple seeds a hierarchical ``SeedSequence``; an
                integer is accepted for backward compatibility.
